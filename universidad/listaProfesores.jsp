...
<html>
  ...
  <table>
    ...
    <% for(int i = 0, n = profs.size(); i < n; i++) {
        ...
        <td><%= prof.getArea() %></td>
        <td><a href='/universidad/detalleProfesor?tp=2&id=<%= prof.getId() %>'>
              <input type="submit" value="Detalle"/></a>
            <a href='/universidad/eliminarProfesor?tp=3&id=<%= prof.getId() %>'>
              <input type="submit" value="Eliminar"/></a></td></tr>
    <% } %>
  </table>
    <table>
      <tr><td><a href='/universidad/agregarProfesor?tp=1'>
